...
<xsl:transform version="1.0"
	...
	<!-- router -->
	<xsl:template match="rdf:RDF/cim:VoltageLevel" mode="c2n:VoltageLevel_2">
		<Router name="{current()/@rdf:ID}"
			RFC1583Compatible="true" />
	</xsl:template>

	<xsl:template match="rdf:RDF/cim:Substation" mode="c2n:Substations_2">
		<Router name="{current()/@rdf:ID}"
			RFC1583Compatible="true" />
	</xsl:template>

	<xsl:template match="rdf:RDF" mode="c2n:VoltageLevelSubRouterNodes_2">
